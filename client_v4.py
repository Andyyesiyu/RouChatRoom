...
class window:
    def __init__(self, root):
        ...
        # photo = tkinter.PhotoImage(file = os.path.join(os.getcwd(),'backgroundgif.gif')) # GIF打开方式
    ...
    def __send_audio_thread(self, audio_data):
        ...
        __audio.close()
        # print(len(audio_data), sys.getsizeof(audio_data[1]))
        # for audio_slice in audio_data:
        #     self.__socket.send(audio_slice)
        print('线程内音频发送完毕')
    ...
    def start(self):
        ...
        启动客户端
        """
        self.__socket.connect((self.ip, 8888))
        root.mainloop()
    ...
    def do_receive_file(self, fn, fd):
        ...
        # 由于文件较大，因此需要分包传送，根据不同文件的大小，进行大小划分
        num = filedata / 1024.0
        if num != int(num):
            num = int(num) + 1
        else:
